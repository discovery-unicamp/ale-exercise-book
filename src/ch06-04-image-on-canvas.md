...
| setScaling    | **a0**: horizontal scaling</br>**a1**: vertical scaling</br>**a7**: 2202 (_syscall_ number)                                                                                                                                                                    | Updates canvas' scaling. **Must be used only after all the canvas' pixels are set.**                                                                                                     |
...
- You can either read the image byte per byte, or use a buffer large enough to store the whole image file content and perform a single read _syscall_.
...
- The setScaling _syscall_ can be used to scale the resulting image, in order to inspect the resulting image. However, it must not be used when running with the assistant.
