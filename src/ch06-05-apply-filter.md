...
- Remember to correctly concatenate the pixel's colors in register **a2** when using the setPixel _syscall_, as explained in [Exercise 6.4](ch06-04-image-on-canvas.md). The output will be in gray scale, so use the same values for the colors (R = G = B) and alpha = 255.
